...
def create_default_registry(file_cache: FileCache | None = None, file_history: Any = None) -> ToolRegistry:
    from autocode.tools.bash import Bash
    from autocode.tools.edit_file import EditFile
    from autocode.tools.file_state_cache import FileStateCache
    from autocode.tools.glob import Glob
    from autocode.tools.grep import Grep
    from autocode.tools.read_file import ReadFile
    from autocode.tools.write_file import WriteFile
...
def register_demo_tools(
    ...
    """注册笔试题演示工具：Calculator / Search / Weather / Todo。

    返回 Todo 实例，便于上层在切换会话时调用 ``set_session()``
    注入当前 session id（见 AutoCodeApp.on_mount）。
    """
    from autocode.tools.calculator import Calculator
    from autocode.tools.search import Search
    from autocode.tools.todo import Todo
    from autocode.tools.weather import Weather

    registry.register(Calculator())
    registry.register(Search())
    registry.register(Weather())
    todo = Todo(storage_dir=storage_dir, session_id=session_id)
